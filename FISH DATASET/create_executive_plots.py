...
"""
    ...
    ax4.text(0.7, 0.1, 'FISH DATASET\nANALYSIS', transform=ax4.transAxes, 
             fontsize=20, fontweight='bold', ha='center', va='center',
             bbox=dict(boxstyle='round,pad=0.5', facecolor='navy', alpha=0.8, edgecolor='gold', linewidth=3))
...
def create_best_model_detailed_plot(df, target_name, save_path):
    ...
    ax2.axhline(y=-std_residual, color='green', linestyle=':', linewidth=2, alpha=0.7)
    ...
    ax3.axvline(y=-std_residual, color='green', linestyle=':', linewidth=2, alpha=0.7)
...
def create_model_comparison_dashboard(df, target_name, save_path):
    """Create a comprehensive dashboard comparing all models."""
    
    # Model results from our analysis
    models_data = {
        'Ridge': {'mae': 1.3074, 'rmse': 1.6858, 'r2': 0.9845, 'cv_mae': 3.4889, 'stability': 'High'},
        'SVR (RBF)': {'mae': 1.3298, 'rmse': 2.0502, 'r2': 0.9770, 'cv_mae': 12.6142, 'stability': 'Medium'},
        'SVR Stable': {'mae': 2.2837, 'rmse': 3.3211, 'r2': 0.9397, 'cv_mae': 20.0387, 'stability': 'Low'},
        'GBR Stable': {'mae': 7.4149, 'rmse': 8.6304, 'r2': 0.5929, 'cv_mae': 13.0726, 'stability': 'Low'},
        'ExtraTrees Stable': {'mae': 5.2448, 'rmse': 6.8894, 'r2': 0.7406, 'cv_mae': 14.7870, 'stability': 'Low'},
        'Random Forest': {'mae': 9.0582, 'rmse': 10.9027, 'r2': 0.3503, 'cv_mae': 17.4266, 'stability': 'Low'}
    }
    ...
    colors = ['#2E8B57', '#32CD32', '#FFD700', '#FF8C00', '#FF6347', '#DC143C']  # Green to Red
    ...
    # Inverse MAE for radar (lower is better)
    mae_norm = 1 - (np.array(maes) / max(maes))
    r2_norm = np.array(r2s)
    ...
    ax1.set_xticklabels(['MAE (Inverted)', 'R² Score'])
    ax1.set_ylim(0, 1)
    ax1.set_title('Model Performance Radar\n(Outer ring = Better performance)', fontsize=18, fontweight='bold', pad=30)
    ax1.legend(loc='upper right', bbox_to_anchor=(1.3, 1.0), fontsize=12)
    
    # Plot 2: Executive Decision Matrix
    ax2 = fig.add_subplot(gs[0, 2])
    ax2.axis('off')
...
"""
    ...
    ax2.text(0.05, 0.95, decision_text, transform=ax2.transAxes, fontsize=14,
             verticalalignment='top', fontfamily='monospace',
             bbox=dict(boxstyle='round,pad=0.5', facecolor='lightcyan', alpha=0.9, 
                      edgecolor='darkblue', linewidth=3))
    
    # Plot 3: Detailed Performance Metrics
    ...
    bars1 = ax3.bar(x_pos - width, maes, width, label='MAE', color='steelblue', alpha=0.8)
    bars2 = ax3.bar(x_pos, r2s, width, label='R² Score', color='orange', alpha=0.8)
    cv_maes = [models_data[model]['cv_mae'] for model in models]
    bars3 = ax3.bar(x_pos + width, [m/max(cv_maes) for m in cv_maes], width, label='CV MAE (Normalized)', color='red', alpha=0.8)
    
    ax3.set_xlabel('Models', fontsize=16, fontweight='bold')
    ax3.set_ylabel('Performance Metrics', fontsize=16, fontweight='bold')
    ax3.set_title('Detailed Performance Comparison\n(All Metrics: Lower = Better, except R²)', fontsize=18, fontweight='bold', pad=20)
    ...
    # Add value labels
    for bars, values in [(bars1, maes), (bars2, r2s)]:
...
"""
    ...
    ax5.text(0.05, 0.95, roadmap_text, transform=ax5.transAxes, fontsize=12,
             verticalalignment='top', fontfamily='monospace',
             bbox=dict(boxstyle='round,pad=0.5', facecolor='lightgreen', alpha=0.9, 
                      edgecolor='darkgreen', linewidth=3))
    
    # Plot 6: Technical Specifications
...
"""
    ...
    ax6.text(0.05, 0.95, specs_text, transform=ax6.transAxes, fontsize=11,
             verticalalignment='top', fontfamily='monospace',
             bbox=dict(boxstyle='round,pad=0.5', facecolor='lightgray', alpha=0.9, 
                      edgecolor='gray', linewidth=3))
    
    # Overall title
    fig.suptitle(f'COMPREHENSIVE MODEL ANALYSIS DASHBOARD\n{target_name}', 
                 fontsize=28, fontweight='bold', y=0.98)
...
def main():
    ...
    dashboard_path = os.path.join(output_dir, 'model_comparison_dashboard.png')
    create_model_comparison_dashboard(df, target_name, dashboard_path)
    
    print(f"\n🎯 EXECUTIVE PLOTS CREATED:")
    print(f"📊 1. Executive Summary: {summary_path}")
    print(f"🔍 2. Detailed Analysis: {detailed_path}")
    print(f"📈 3. Comprehensive Dashboard: {dashboard_path}")
